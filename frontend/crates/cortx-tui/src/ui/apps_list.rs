...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    let title = format!(" Apps ({}) ", app.apps_filtered_indices.len());
    ...
    for (fi, &idx) in app.apps_filtered_indices.iter().enumerate() {
        let core_app = &app.apps[idx];
        let primary_tag = core_app.tags.first().map(|s| s.as_str());
        ...
        spans.push(Span::styled(&core_app.name, Style::default().fg(theme::TEXT_PRIMARY)));

        let line = Line::from(spans);
        items.push(ListItem::new(line));
    }

    display_selected += app.apps_selected_index;

    let mut state = ListState::default();
    if !app.apps_filtered_indices.is_empty() {
