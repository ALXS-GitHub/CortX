...
| P2 | Implémentée (2026-09-03), à valider | `sessions.json` écrit par le Rust à chaque révision ; cwd par pane via OSC 7 (écrit par la fenêtre si ouverte, sinon par la principale) ; `windowOpen` patché par le backend à l'ouverture / fermeture ; reprise dans la fenêtre principale au boot (shells recréés avec `restoreFrom`, ids remplacés en un seul commit) ; instantanés `runtime/terminal-snapshots/*.bin` (quit + toutes les 60 s), rejoués via le hub avant la sortie du nouveau shell ; launch configs YAML validées côté Rust, exécutées côté front (commandes tapées dans le shell après 1,2 s) ; `--layout` sur `cortx terminal`. Le dock n'est pas restauré (ses onglets sont surtout des services relançables). Retours d'Alexis intégrés : instantanés sérialisés par xterm (plus de débordement de couleurs ni de redessin décalé), palette Ctrl+K de la fenêtre (launch configs hors des boutons), zoom Ctrl+= / - / 0, panes sans en-tête ni barre d'état, rail avec drag and drop, couleur, épingle, renommage. |
| P3 | Implémentée (commit `332b0e1`, 2026-09-03), à valider | Registre de raccourcis façon Warp configurables (Réglages > Shortcuts), recherche, pane maximisé, numéros d'onglets, réouvrir, dupliquer, copier le chemin, glisser-déposer de fichiers ; suggestions fantômes (OSC 133 côté xterm, historique commun, → accepte ; PSReadLine coupé dans CortX) ; thèmes format Warp (8 intégrés, import fichier/dossier, `aespa_wda` importé chez Alexis), sélecteur avec aperçu, thème sur toute la fenêtre + fond d'écran + opacité + acrylic/mica (fenêtre transparente), curseur, padding. Vérifié via CDP : Dracula et aespa appliqués à toute la fenêtre, fantôme accepté, split par raccourci, barre de recherche. Réglages « look » ajoutés à la demande d'Alexis : opacité / flou / ajustement / assombrissement du fond d'écran, opacité et flou de la barre de titre et du rail, opacité de fenêtre dès 20 %. |
| P4 | À faire | Blocs (navigation prompt à prompt, replier, copier), recherche globale multi-sessions, palette Ctrl+R historique + alias + scripts, détection d'agents dans le rail. |
