...
| F4.3.39 | Scrollback snapshots: each window serialises its xterm buffers (plain lines with colours, no cursor movement, N lines, 256 KB cap) to `runtime/terminal-snapshots/` every 30 s and when the app starts closing; the backend's raw PTY tail is only a fallback when no fresh GUI snapshot exists. A restored shell replays the snapshot (attributes reset on both sides) before its own output, under a "restored session" rule (`restoreScrollback`, `restoreScrollbackLines`) | Should Have | Done |
| F4.3.42 | Terminal window palette (Ctrl+K / Ctrl+Shift+P): new terminal, splits, send to dock, close pane, zoom, rail, scope, run / save launch configurations — the rail and panes stay free of buttons | Should Have | Done |
| F4.3.43 | Zoom: Ctrl+= / Ctrl+- / Ctrl+0 change the font size of every terminal of the window (per window, not persisted) | Should Have | Done |
| F4.3.44 | Lean panes: no per-pane header or status bar; the rail row carries the live cwd (or the running command); pane actions appear on hover; rail tabs reorder by drag and drop and take a colour, pin and rename from a context menu | Should Have | Done |
| F4.3.45 | Keybindings (DEV-13 P3): a registry of rebindable actions with Warp-like defaults — new tab in the focused pane's cwd, close pane, next / previous tab, Ctrl+1-9, rename, reopen closed tab, split right / down, focus and resize panes, maximize pane, find, clear, zoom, palette, rail — stored in `settings.terminal.keybindings`, edited in Settings > Shortcuts (record a chord, conflicts flagged) | Must Have | Done |
| F4.3.46 | Productivity: find in terminal (Ctrl+Shift+F, highlighted matches), maximize pane, tab numbers while Ctrl is held, reopen closed tab, duplicate tab, copy path / open in Explorer from the tab menu, dropping files pastes their quoted paths | Should Have | Done |
| F4.3.47 | Inline suggestions: ghost text from the cross-terminal command history at the cursor, computed on the xterm side from the OSC 133 markers (→ accepts, Esc hides; off in TUIs and mid-line); `cortx init` disables PSReadLine's own prediction inside CortX so only one suggestion shows (`inlineSuggestions`) | Should Have | Done |
| F4.3.48 | Themes in Warp's YAML format under `data/terminal/themes/` (synced): 8 bundled (Dark / Light Modern, Halcyon Dark / Light, Dracula, Nord, Solarized Dark, Gruvbox Dark), import a file or a whole folder (Warp's themes repository works as is), `cortx:` extensions (cursor, selection, blur, image fit), picker with live preview (palette, Settings), one theme per light / dark mode or a fixed one | Must Have | Done |
| F4.3.49 | The theme drives the whole Terminal window like Warp — title bar, rail, panes — with the wallpaper behind everything; window opacity 20-100 and acrylic / mica / vibrancy backdrop on a transparent window; cursor style, blink and padding | Must Have | Done |
| F4.3.50 | Look overrides on top of the theme file, all in `settings.json`: wallpaper opacity, blur, fit and a dimming overlay in the theme colour; title bar + rail background opacity and frosted blur (`wallpaper*`, `chromeOpacity`, `chromeBlur`), with live sliders in Settings > Terminal appearance | Should Have | Done |
| F4.3.51 | App-side shell integration: the app hands the OSC 7 / 133 block to every shell it spawns as start-up code after the profile (PowerShell `-NoExit -Command`, bash `--rcfile`, zsh private `ZDOTDIR`, fish `-C`), so cwd, command status and suggestions work whether or not the profile calls `cortx init`; PTYs never inherit the launcher's Claude Code session markers | Must Have | Done |
| F4.3.52 | Terminal window comfort: an empty scope opens a shell automatically, Ctrl + wheel zooms, thin rounded scrollbar, "CortX Terminal" as a terminal-application preset (external launches open in the Terminal window), optional terminal theme on the dock (`dockUsesTerminalTheme`) | Should Have | Done |
...
| F5.14 | Text rendering settings: renderer — Canvas (default: box / block / powerline characters drawn at the cell size, text rasterised by the platform), GPU (fastest on heavy output) or Browser (no acceleration) — font weight and bold weight, letter spacing, line height, and the terminal selection colour | Should Have | Done |
